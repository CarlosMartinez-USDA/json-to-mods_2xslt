...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:template match="data">
        <data>
            <xsl:result-document method="xml" omit-xml-declaration="yes"
                href="{$working_dir}/archive-files/B-{$original_filename}_{position()}.json"
                format="archive">
        ...
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            format="original"
            href="{$working_dir}/mods-files/N-{$original_filename}_{position()}.xml">
            <mods version="3.7">
    ...
    <xd:doc>
        <xd:desc>Match prioritisation acronym, and uses two extermal stylesheets to provide a whole
    ...
    <xsl:template match="map/string[@key = 'modified_on' or 'created_on']"
    ...
    <xsl:template name="part" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <part>
            ...
            <!--extent/pages-->
            <xd:doc>
                <xd:desc>Transforms, in order of preference, the publication-related date
                    metadata</xd:desc>
            </xd:doc>
            <xsl:choose>
                <xsl:when test="(./string[@key = 'modified_on'] != ' ')">
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" mode="special_cases"
        xpath-default-namespace="http://www.w3.org/2005/xpath-functions" name="special_cases">
        <xsl:param name="citation"/>
        <xsl:variable name="lastNumber"
            select="string(tokenize(/fn:map/fn:string[@key = 'citation'], '[^\d]+')[.][last()])"/>
        <xsl:variable name="secondToLastNumber"
