    ...
    <xsl:template match="data">
       <data> 
           <xsl:result-document method="xml" omit-xml-declaration="yes"
                href="{$working_dir}/workingDir_for_11-22-2021_xsl/archive-results/A-{$original_filename}_{position()}.json" format="archive">
                <xsl:value-of disable-output-escaping="yes" select="."/>
            </xsl:result-document>
       </data>
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            format="original" href="{$working_dir}/workingDir_for_11-22-2021_xsl/mods-results/N-{$original_filename}_{position()}.xml">
            <mods version="3.7">
...
<!--
  ...
  <xsl:variable name="citation" select="./string[@key = 'citation']"/>
        <part>
            ...
            <extent unit="pages">
               <xsl:choose>
                    <xsl:when test="matches(./string[@key = 'pub_publication'], '(\d+-\d+)')">
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'pub_publication']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions" 
        ...
        <xsl:choose>
            <xsl:when test="matches($pub_publication, concat($secondToLastNumber, '-', $lastNumber))"> 
                <start>
