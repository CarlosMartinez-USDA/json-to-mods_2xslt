    ...
    <xsl:template match="data">
        <data>
            <xsl:result-document method="xml" omit-xml-declaration="yes"
                href="{$working_dir}B-{$original_filename}/archive-results/{position()}.json" format="archive">
                <xsl:value-of disable-output-escaping="yes" select="."/>
            </xsl:result-document>
        </data>
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            format="original" href="{$working_dir}N-{$original_filename}/mods-results/{position()}.xml">
            <mods version="3.7">
