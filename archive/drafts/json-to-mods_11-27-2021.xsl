...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <relatedItem type="host">
                    <xsl:apply-templates select="map/string[@key = 'pub_publication']"
                        mode="related_item"/>
                    <xsl:call-template name="part"/>
                </relatedItem>
            </xsl:otherwise>
    ...
    <!--pub_desc_type to relatedItem/titleInfo-->
    <xd:doc>
        <xd:desc>
    ...
    <xsl:template match="map/string[@key = 'pub_type_desc']" mode="relatedItem"
        xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <relatedItem type="host">
            <titleInfo>
                <title>
    ...
    <!--pub_publication to relatedItem/titleInfo-->
    <xd:doc>
        <xd:desc>
            <xd:p>Publication info as a string to be parsed</xd:p>
            <xd:p>Journal host info: base doi, origin, agency, sub-agency, research station,
    ...
    <xsl:template match="map/string[@key = 'pub_publication']" mode="related_item"
        xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
    ...
    <!--mods:part - details: volume, issue, page numbers-->
    <xd:doc>
        <xd:desc/>
    </xd:doc>
...
<!--    <!-\-PUB-PUBLICATION-\->
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'pub_publication']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions" 
        ...
        <xsl:variable name="secondToLastNumber" select="string(tokenize(/fn:map/fn:string[@key = 'pub_publication'], '[^\d]+')[.][last()-1])"/>   
        <xsl:choose>
            <xsl:when test="contains($pub_publication, '-')"> 
                <total>
                    <xsl:value-of select="$lastNumber"/>
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" mode="start_end_total" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$citation">
                <xsl:apply-templates select="/fn:map/fn:string[@key = 'citation']" mode="analyze-string"/>
            </xsl:when>
            <xsl:otherwise><!--
                <xsl:apply-templates select="/fn:map/fn:string[@key = 'citation']" mode="total_only"
                />-->
                <xsl:call-template name="total_only"/>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" mode="total_only" xpath-default-namespace="http://www.w3.org/2005/xpath-functions"
        name="total_only">
        <xsl:param name="citation"/>
        <xsl:variable name="lastNumber"
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="special_cases"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" mode="analyze-string" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:param name="citation"/>
        <xsl:variable name="lastNumber"
            select="string(tokenize(/fn:map/fn:string[@key = 'citation'], '[^\d]+')[.][last()])"/>
        <xsl:variable name="secondToLastNumber"
        ...
        <xsl:if test="matches($citation, '[^\d+](\d+\sp)')">
            <xsl:choose>
                <xsl:when test="matches($citation, '[A-z]?(/d+)(-)[A-z]?(\d+)')">
                    
                    <xsl:analyze-string select="$citation" regex="(\d+)(-)(\d+)">
                        <xsl:matching-substring>
                            <start>
                            ...
                            <total>
                                <xsl:value-of select="f:calculateTotalPgs(regex-group(1), regex-group(2))"/>
                            </total>
                        </xsl:matching-substring>
                ...
                <xsl:when test="matches($citation, $lastNumber)">
                    <total>
                        <xsl:value-of select="$lastNumber"/>
                    </total>
                </xsl:when>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:ul>
                <xd:li>
                    <xd:p>Test One: Special case for pages starting with the letter "s"</xd:p>
                </xd:li>
            </xd:ul>
        </xd:desc>
        ...
        <xd:param name="citation"/>
        <xd:param name="alpha_translate"/>
        <xd:param name="pub_publication"/>
        <xd:param name="pub_page"/>
    </xd:doc>
    <xsl:template name="pages" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:param name="pub_page" select="string[@key = 'pub_page']"/>
        ...
        <xsl:param name="alpha_translate"
            select="'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuzwxuyz'"/>
        <xsl:choose>
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                ...
                <xsl:analyze-string select="$citation"
                    regex="(Pages\s)(\d+-\d+)|(\d+)(\sp)|(:\s\d+)(-/d+)?">
                    <xsl:matching-substring>
                        <xsl:choose>
                            <xsl:when test="regex-group(3)">
                                <xsl:comment>test 3a</xsl:comment>
                                <total>
                                    <xsl:value-of select="regex-group(3)"/>
                                </total>
                            </xsl:when>
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:choose>
                            <xsl:when test="matches($citation, '[^\d+](\d+\-\d+)')">
                                <xsl:comment>test 3d</xsl:comment>
                                <xsl:choose>
                                    <xsl:when
                                        test="matches($citation, '(\d+-\d+)') and contains($citation, '-')">
                                        <xsl:apply-templates select="$citation"
                                            mode="start_end_total"/>
                                    </xsl:when>
                                    <xsl:when test="matches($citation, '\d+\s\p|(\d+)(-\d+)?')">
                                        <xsl:apply-templates select="$citation" mode="total_only"/>
                                    </xsl:when>
                                    <xsl:otherwise>
            ...
            <xsl:when test="contains(/map/string[@key = 'pub_page'], '-')">
                <xsl:comment>test 4</xsl:comment>
                <extent unit="pages">
                    <xsl:analyze-string select="/map/string[@key = 'pub_page']"
                        regex="(\d+)(\-)(\d+)">
                        <xsl:matching-substring>
            ...
            <xsl:when test="contains(/map/string[@key = 'pub_page'], '-')">
                <xsl:comment>test 5</xsl:comment>
                <extent unit="pages">
                    <xsl:analyze-string select="/map/string[@key = 'pub_page']"
                        regex="(\d+)(\-\d+)?">
                        <xsl:matching-substring>
                            <xsl:choose>
                                <xsl:when test="regex-group(1) and regex-group(2)">
                                    <xsl:variable name="strpGroup2"
                                        select="replace(regex-group(2), '(-)(\d+)', '$2')"/>
                                    <start>
                                        <xsl:number value="regex-group(1)"/>
                                    </start>
                                    <end>
                                        <xsl:number value="$strpGroup2"/>
                                    </end>
                                    <total>
                                        <xsl:value-of
                                            select="f:calculateTotalPgs(number(regex-group(1)), number($strpGroup2))"
                                        />
                                    </total>
                                </xsl:when>
                            </xsl:choose>
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                <xsl:comment>test 6</xsl:comment>
                    <extent unit="pages">
                        <xsl:analyze-string select="$citation" regex="([A-z]\d+)-([A-z]\d+)|(\d+)-(\d+)|(\d+)(\sp|\sp&#x22;)">
                        <xsl:matching-substring>
                            <xsl:choose>
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                <xsl:comment>test 7</xsl:comment>
                <xsl:variable name="citation_string"
                    select="local:reverse-string(map/string[@key = 'citation'])"/>
                <xsl:variable name="citation_pages"
                    select="tokenize($citation_string, '(\d+|\d+\-\d+)')"/>
                <xsl:choose>
                    <xsl:when test="contains($citation_pages, '-')">
                        <extent unit="pages">
                            <start>
                                <xsl:value-of
                                    select="replace($citation_pages, '(\d+)(\-)(\d+)', '$1')"/>
                            </start>
                            <end>
                                <xsl:value-of
                                    select="replace($citation_pages, '(\d+)(\-)(\d+)', '$3')"/>
                            </end>
                            <total>
                                <xsl:value-of
                                    select="f:calculateTotalPgs(replace($citation_pages, '(\d+)(\-)(\d+)', '$1'), replace($citation_pages, '(\d+)(\-)(\d+)', '$3'))"
                                />
                            </total>
                        </extent>
                    </xsl:when>
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                <xsl:comment>test 8</xsl:comment>
                <xsl:analyze-string select="$citation"
                    regex="\D(\d+-\d+)(\s|\.)|(\d+)\sp|\]\spages\.|$\n|(\d\.\d+-\d\.\d+)">
                    <xsl:matching-substring>
                        <xsl:choose>
                            <xsl:when test="regex-group(3)">
                                <total>
                            ...
                            <xsl:when test="regex-group(4)">
                                ...
                                <total>
                                    <xsl:variable name="first"
                                        select="number(substring-before(regex-group(4), '-'))"/>
                                    <xsl:variable name="last"
                                        select="number(substring-after(regex-group(4), '-'))"/>
                                    <xsl:value-of select="f:calculateTotalPgs($first, $last)"/>
                                </total>
                            </xsl:when>
                            <xsl:otherwise>
