...
<xsl:key name="pages" match="map/string" use="@key"/>
    ...
    <xd:doc>
        <xd:desc>
            <xd:p><xd:b>'pub_authors'</xd:b> array contains the key values for author name's and uses numbers and
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" mode="start_end_total" xpath-default-namespace="http://www.w3.org/2005/xpath-functions"
        ...
        <xsl:choose>
            ...
            <xsl:when test="$citation">
                <xsl:apply-templates select="/fn:map/fn:string[@key = 'citation']" mode="analyze-strin"/>
            </xsl:when>
            <xsl:otherwise>
