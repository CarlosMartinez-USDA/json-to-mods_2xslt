...
<xsl:key name="pages" match="map/string" use="@key"/>
    ...
    <xsl:template name="name-info" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:if test="./string[@key = 'station_id'] != ''">
            <affiliation>
                ...
                <xsl:text>, </xsl:text>
                <xsl:choose>
                    <xsl:when test="number(./string[@key = 'unit_id']) > 0 and ./string[@key = 'unit_id'] !='NaN'">
...
<!--                        <xsl:comment>test 1: pub_page field</xsl:comment>-->
            ...
            <xsl:when test="/map/string[@key = 'citation']|/map/string[@key = 'pub_publication']">
                <xsl:variable name="alpha-translate" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'"/>
                    
                <xsl:comment>test 3a: citation field, 1st iteration</xsl:comment>

                <xsl:analyze-string select="$citation" regex="([A-z]\d+)-([A-z]\d+)|(\d+)-(\d+)|(\d+)\sp.?">
                    <xsl:matching-substring>
                        <xsl:choose>
                           ...
                           <xsl:when test="regex-group(1) and regex-group(2)">
                                ...
                                <total>
                                   <xsl:value-of select="f:calculateTotalPgs(translate(regex-group(1),$alpha-translate, ''), translate(regex-group(1),$alpha-translate, ''))"/>
                                </total>
                           </xsl:when>
                            <xsl:when test="regex-group(3) and regex-group(4)">
                                    ...
                                    <total>
                                       <xsl:value-of select="f:calculateTotalPgs(regex-group(3), regex-group(4))"/>
                                    </total>
                            </xsl:when>
                            <xsl:otherwise>
