...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:template match="data">
        <data>
            <xsl:result-document method="xml" omit-xml-declaration="yes"
                href="{$working_dir}/archive-files/B-{$original_filename}_{position()}.json" format="archive">
    ...
    <xd:doc scope="component" id="main_title">
        <xd:desc><xd:p><xd:b>JSON to MODS title/titleInfo transformation</xd:b></xd:p>
    ...
    <xsl:template match="map/array[@key = 'pub_authors'] | map/array[@key = 'primary_station']"
        xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <xsl:choose>
            <xsl:when test="map/array[@key = 'primary_station'] and count(map/string[@key = 'name']) = 0">
                <name type="corporate">
                    <namePart>
...
<!--MODS TAGS: [namePart @given, @family], [displayForm], [affiliation] --> 
    <xd:doc>
        <xd:desc>
    ...
    <xsl:template name="dateIssued" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:template match="/map/string[@key='pub_publication']/">
            <output>
                <xsl:for-each select="/map/string[@key='pub_publication']">
                    <xsl:element name="{name()}">
                        <xsl:call-template name="getEndNumber">
                            <xsl:with-param name="input" select="normalize-space(.)"/>
                            <xsl:with-param name="dotIncluded" select="true()"/>
        ...
        <xsl:template name="getEndNumber">
            ...
            <xsl:choose>
                ...
                <xsl:when test="substring($input, $len, 1) = '.' and $hyphen">
                    <xsl:call-template name="getEndNumber">
                        <xsl:with-param name="input" select="substring($input, 1, $len - 1)"/>
    ...
    <xsl:template name="relatedItem"
        ...
        <xsl:variable naame="citation" select="./string[@key = 'citation']"/>
        <xsl:choose>
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" name="start_end_total" mode="start_end_total" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
    ...
    <xsl:template match="map/string[@key='citation']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions"
        name="analyze-citation ">
        <xsl:analyze-string select="map/string[@key='citation']" regex="([A-z])(\d+)-([A-z])(\d+)|(\d+-\d+)|(\d+)\sp">
       <xsl:matching-substring>
           <xsl:choose>
               <xsl:when test="matches(.,'[A-z](\d+)-[A-z](\d+)')">
                   ...
                   <total>
                       <xsl:value-of select="f:calculateTotalPgs(regex-group(2), regex-group(4))"/>)"
                   </total>
               </xsl:when>
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" mode="special_cases">
        ...
        <xsl:choose>
            <xsl:when test="$lastNumber and $secondToLastNumber">
                <xsl:if test="matches($citation, '[A-z]?\d+-[A-z]?\d+')">
                    <xsl:comment>subtest3.b.iii</xsl:comment>
                    <start>
                        <xsl:value-of select="$secondToLastNumber"/>
                    </start>
                    <end>
