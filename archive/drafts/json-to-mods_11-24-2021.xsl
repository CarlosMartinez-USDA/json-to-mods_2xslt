...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:template match="data">
            ...
            <xsl:result-document method="xml" omit-xml-declaration="yes"
                href="{$working_dir}/workingDir_for_11-24-2021_xsl/archive-results/A-{$original_filename}_{position()}.json" format="archive">
                <xsl:value-of disable-output-escaping="yes" select="."/>
            </xsl:result-document>
        
        <xsl:result-document method="xml" indent="yes" encoding="UTF-8" media-type="text/xml"
            format="original" href="{$working_dir}/workingDir_for_11-24-2021_xsl/mods-results/N-{$original_filename}_{position()}.xml">
            <mods version="3.7">
    ...
    <xsl:template match="map" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <accessCondition type="use and reproduction" displayLabel="Resource is Open Access">
            <program xmlns="https://data.crossref.org/schemas/AccessIndicators.xsd">
                <license_ref>
                    <xsl:comment>http://purl.org/eprint/accessRights/OpenAccess</xsl:text>
                </license_ref>
            </program>
        </accessCondition>     
    ...
    <xsl:template match="map/array[@key = 'pub_authors'] | map/array[@key = 'primary_station']"
        ...
        <xsl:choose>
            <xsl:when test="count(map/string[@key = 'name']) = 0">
                <name type="corporate">
                    <namePart>
                        <xsl:comment>United States Department of Agriculture, Forest Service, </xsl:text>
  ...
  <xsl:variable name="citation" select="./string[@key = 'citation']"/>
            ...
            <extent unit="pages"/>
                <xsl:choose>
                    <xsl:when test="./string[@key = 'pub_publication'] !=''"> 
                        <xsl:comment>test pub-publication</xsl:comment>
    ...
    <xsl:template name="test">
        <xsl:value-of select="tokenize(/fn:map/fn:string[@key = 'pub_publication'], '[^\d]+')[.][last()]"/>
    </xsl:template>
    <!--PUB-PUBLICATION-->
    <xd:doc>
        <xd:desc/>
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'pub_publication']" xpath-default-namespace="http://www.w3.org/2005/xpath-functions" 
        ...
        <xsl:choose>
            ...
            <xsl:when test="matches($pub_publication,'(\d+\sp)')">
                <total>
                    <xsl:value-of select="$lastNumber"/>
                </total>        
            </xsl:when>
    ...
    <xsl:template match="/fn:map/fn:string[@key = 'citation']" mode="start_end_total">
        ...
        <xsl:choose>
            <xsl:when test="matches($citation, '[^\d+](\d+\-\d+)')">
                <xsl:comment>subtest3.2</xsl:text>
                <start>
                    <xsl:value-of select="$secondToLastNumber"/>
                </start>
                <end>
        ...
        <xsl:choose>
            <xsl:when test="$lastNumber and $secondToLastNumber">
                <xsl:if test="matches($citation, '[A-z]\d+-[A-z]\d+')">
                    <xsl:comment>subtest3.b.iii</xsl:text>
                    <start>
                        <xsl:value-of select="$secondToLastNumber"/>
                    </start>
                    <end>
    ...
    <xsl:template name="pages" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:choose>
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                    ...
                    <xsl:analyze-string select="$citation" regex="(Pages\s)(\d+-\d+)|(\d+)(\sp)">
                        <xsl:matching-substring>
                            <xsl:choose>
                        ...
                        <xsl:matching-substring>
                            <xsl:choose>
                                <xsl:when test="regex-group(1) and regex-group(2)">
                                    <xsl:variable name="strpGroup2" select="replace(regex-group(2), '(-)(\d+)', '$2')"/>
                                    <start>
                                        <xsl:number value="regex-group(1)"/>
                                    </start>
                                    <end>
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                    ...
                    <xsl:analyze-string select="$citation" regex="((\d+-\d+)|(\d+)(\sp))">
                        <xsl:matching-substring>
                            <xsl:choose>
                                <xsl:when test="regex-group(4)">
                                    <total>
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                ...
                <xsl:variable name="citation_string" select="local:reverse-string(map/string[@key = 'citation'])"/>
                <xsl:variable name="citation_pages" select="tokenize($citation_string, '(\d+|\d+\-\d+)')"/>
                <xsl:choose>
                    <xsl:when test="contains($citation_pages, '-')">
                       
                            <start>
                                <xsl:value-of select="replace($citation_pages, '(\d+)(\-)(\d+)', '$1')"/>
                            </start>
                            <end>
                                <xsl:value-of select="replace($citation_pages, '(\d+)(\-)(\d+)', '$3')"/>
                            </end>
                            <total>
                                <xsl:value-of
            ...
            <xsl:when test="/map/string[@key = 'citation']">
                    ...
                    <xsl:analyze-string select="$citation" regex="\D(\d+-\d+)(\s|\.)|(\d+)\sp|\]\spages\.|$\n|(\d\.\d+-\d\.\d+)">
                        <xsl:matching-substring>
                            <xsl:choose>
   ...
   --> <xd:doc>
    ...
    <xsl:template name="identifiers"
        ...
        <xsl:if test="/map/string[@key = 'doi']">
            ...
            <location>
                <url displayLabel="Direct access to publisher’s site">
                    <xsl:comment>http://dx.doi.org/</xsl:text>
        ...
        <xsl:if test="/map/string[@key = 'treesearch_pub_id']">
            ...
            <location>
                <url displayLabel="Direct access to publisher’s site">
                    <xsl:comment>https://www.fs.usda.gov/treesearch/pubs/</xsl:text>
    ...
    <xsl:template name="extension" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        <!--development and production server parameters-->
        <extension>
            <vendorName>
            ...
            <!--localized testing variables-->
            <xsl:comment>Global Variable Values</xsl:comment>
            <archive_file>
