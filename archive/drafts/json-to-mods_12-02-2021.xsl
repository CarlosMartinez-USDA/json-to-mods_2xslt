...
<!DOCTYPE transform [
         ...
         "file:/C:/Users/carlos.martinez/Documents/json-to-xml-master/ent/predefined.ent"
       >
       %predefined;
]>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:usfs="http://usfs_treesearch" xmlns:local="http://local_functions"
    ...
    <xsl:template match="data">
        <xsl:result-document omit-xml-declaration="yes" indent="yes" encoding="UTF-8"
            href="file:///{$workingDir}B-{replace($originalFilename, '(.*/)(.*)(\.json)','$2')}_{position()}.json" format="archive">
            <xsl:copy-of select="."/>
        </xsl:result-document>
...
<!--    <xd:doc>
                ...
                <xsl:if test="count($acronym) > 0">  
                    <affiliation>
                        ...
                        <xsl:choose>
                           ...
                           <xsl:text>,</xsl:text>
                           <xsl:value-of select="local:unitNumberToName($unitNum[current()]/text())"/>                          
                       </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>,</xsl:text>
                            <xsl:value-of select="local:unitAcronymToName($unitAcronym)"/>                           
                        </xsl:otherwise>
                        </xsl:choose>
                        <xsl:text>,</xsl:text>
    ...
    <xsl:template match="" xpath-default-namespace="http://www.w3.org/2005/xpath-functions" mode="get_value">
<titleInfo type="abbreviated">
        <title>
        <xsl:value-of select="."/>
        </title>
    ...
    <xd:doc>
        <xd:desc><xd:p>mods:relatedItem/<xd:b>mods:part</xd:b></xd:p></xd:desc>
    </xd:doc>
